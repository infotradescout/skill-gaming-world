...
test("public landing page states the noncash product boundary", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Play Coins have no cash value and cannot be withdrawn/),
  ).toBeVisible();
  await expect(
...
test("account access, held modes, and Play Coin exact retry remain coherent", async ({
  ...
  await expect(page.getByText(/Safe demo · noncash entertainment/)).toBeVisible();
...
test("casino and privileged admin surfaces remain held", async ({ page }) => {
  ...
  await expect(page.locator(".casino-balance strong")).toHaveText(
    /HARD\s+HOLD/,
  );
  await expect(
    page.getByText(

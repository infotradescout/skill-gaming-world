...
try{
 ...
 const receipt=await page.evaluate(async()=>{const r=await fetch('art03/art_receipt.json');return r.json()});report.sourceCommit=receipt.source_commit;report.artRevision=receipt.revision;
 ...
 // Only the diagnostic transport is reduced. The PNG and desktop capture retain
 // their original resolution; this thumbnail cannot establish fine-detail approval.
 const jpeg=await page.locator('#view').evaluate(i=>{const c=document.createElement('canvas');c.width=320;c.height=180;c.getContext('2d').drawImage(i,0,0,320,180);return c.toDataURL('image/jpeg',.18).split(',')[1]});
 await fs.writeFile(path.join(root,'acceptance03/native-review.jpg'),Buffer.from(jpeg,'base64'));
 console.log('ART031_THUMB_BYTES '+Buffer.from(jpeg,'base64').length);
 for(let i=0;i<jpeg.length;i+=2048)console.log('ART031_THUMB_'+String(i/2048).padStart(2,'0')+' '+jpeg.slice(i,i+2048));
 ...
 const head=await page.request.head(origin+'/art03/Wildlife_Lodge_Shore_03.blend');if(head.status()!==200)throw Error('Native file unavailable');report.checks.push('native scene downloadable');report.passed=true;

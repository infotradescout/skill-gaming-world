"""Download a bounded set of CC0 authoring assets via Poly Haven's public API.
The live API is used only in the build, with a unique User-Agent and attribution.
No account, paid library, game extraction or third-party executable is involved.
"""
import concurrent.futures, hashlib, json, os, sys, time, urllib.request, urllib.parse
...
def request(url):
 ...
 if u.scheme!='https' or u.hostname not in {'api.polyhaven.com','dl.polyhaven.org','dl.polyhaven.com','wildlife-reserve-world-preview.onrender.com'}:
  raise ValueError('Asset host not approved: '+url)
...
def prepare():
 ...
 for slug in MODEL_IDS+TEXTURE_IDS:
  ...
  (folder/'api_metadata.json').write_text(json.dumps(metadata,indent=2))
  (folder/'api_files.json').write_text(json.dumps(files,indent=2))
  if slug in MODEL_IDS:
   ...
   for rel,entry in item.get('include',{}).items():
    tasks.append((entry['url'],safe_path(folder,rel),entry.get('md5'),None,entry.get('size')))
   ...
   for channel in ['diff','nor_gl','rough','disp']:
    item,res=variant(files[channel],'2k','jpg')
    name=urllib.parse.unquote(Path(urllib.parse.urlsplit(item['url']).path).name)
    tasks.append((item['url'],folder/name,item.get('md5'),None,item.get('size')))
    maps[channel]=str((folder/name).relative_to(ROOT))
 ...
 # Preserve the exact previously authored district, not a rebuilt approximation.

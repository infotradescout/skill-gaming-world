...
describe("Platynum installer download", () => {
  beforeEach(() => {
  ...
  it("requires the owner gate before opening the installer", async () => {
  ...
  it("downloads only the fixed installer artifact with private response headers", async () => {
    ...
    expect(response.headers.get("content-disposition")).toBe(
      'attachment; filename="Platynum-47-Setup-0.2.0.exe"',
    );
